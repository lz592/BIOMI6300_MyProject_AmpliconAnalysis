...
- 'Sample.Name' includes oxygen-related indicators (e.g., 20O2, 10O2, 5O2).
...
Microbial alpha diversity does vary across different oxygen concentrations, potentially decreasing under lower oxygen conditions due to shifts in community composition.
...
# Extract oxygen level (e.g., 20 from "AME-D2-20O2-16s")
metadata_df <- 
  metadata_df %>%
...
There are no environmental variables in the metadata of this research, and the only factor that affect the biodiversity is oxygen level provided into the bioreactor. 
