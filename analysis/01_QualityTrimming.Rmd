...
Therefore, I don’t need to use the ‘trimLeft’ parameter to remove the first several bases in both forward and reverse reads.
2. High Quality in Mid-Read (~Cycle 10-250)
The sequencing quality is stable and above Q30 across most of the sequencing cycle/read (Cycle 10-300). This suggests that most of the read length is high-quality and can be retained.
3. Decline in Reverse Reads After ~200 Cycles
The reverse reads exhibit a notable drop in quality after ~210 cycles, dropping below Q30.
The decline is more severe in reverse reads than in forward reads, indicating a high sequencing error rate. 
This suggests that truncating the reverse reads around Cycle 200-220 might improve quality. Therefore, I will truncate the read using ‘truncLen = 210’ to remove those final 90 poor-quality bases at the end of the read. The beginning of the read is ~30 so we are ok at the beginning of the read.
4. Forward Reads Maintain Quality Slightly Longer (~Cycle 230-300)
The forward reads show a more gradual decline in quality after Cycle 230, but they remain above Q30 longer than the reverse reads. The final cycles (~280-300) show a steep drop in quality, which is common for Illumina sequencing. 
...
filtered_reads <- 
  filterAndTrim(fwd = forward_reads, filt = filtered_forward_reads,
              ...
              maxN = 0,
              # The sequence quality was not bad, so I choose to use maxEE = c(2,2) to make it better
              maxEE = c(2,2), 
              # Don't need to remove the first several bases at the beginning of both forward and reverse reads
              trimLeft = c(0, 0),
              # Trim final 20 bases in the forward & trim final 90 bases in the reverse
              truncLen = c(280, 210), 
...
Most samples retained between ~10,000 and 20,000 or between ~35,000 and 55,000 sequences, indicating that the majority of data was preserved. A small number of samples have lower retention, but there is no drastic loss.
...
•	Beginning of read: No base was trimmed at the beginning of both forward and reverse reads, which makes sense due to good initial quality (~Q35). 
